...
export default function Home() {
  ...
  const [paquetesActivos, setPaquetesActivos] = useState<number>(0);
  const [paquetesTransito, setPaquetesTransito] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const activos = packages.filter(
          (p) => p.status_name?.toUpperCase() !== "ENTREGADO"
        ).length;

        const enTransito = packages.filter((p) => {
          const st = (p.status_name || "").toUpperCase();
          return (
            st === "EN TRANSITO A MIAMI" ||
            st === "EN TRANSITO A PANAMA"
          );
        }).length;

        const existePorConfirmar = packages.some(
        ...
        setPaquetesTransito(enTransito);
        setConductoresActivos(totalDrivers);
        ...
        setPaquetesActivos(0);
        setPaquetesTransito(0);
        setConductoresActivos(0);
        setTotalClientes(0);
        setTotalPaquetes(0);
  ...
  return (
    <>
      <PageMeta
        title="Panel Operativo - Get"
        description="Dashboard administrativo y métricas operativas"
      />
      ...
      <div className="p-4 md:p-6 flex flex-col gap-6">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-white/[0.05] dark:bg-white/[0.03]">
            ...
            <p className="text-[13px] text-gray-500 dark:text-white/50 mt-1">
              Rumbo a Panamá / Miami
            </p>
          </div>

          {/* Conductores Activos */}
          <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm text-left dark:border-white/[0.05] dark:bg-white/[0.03]">
            <p className="text-xs font-medium text-gray-500 dark:text-white/60 uppercase tracking-wide">
              Conductores Activos
            </p>
            <p className="mt-2 text-3xl font-semibold text-gray-900 dark:text-white">
        ...
        {hayPorConfirmar && (
          <section className="rounded-2xl border border-amber-200 bg-amber-50 p-4 shadow-sm dark:border-amber-500/30 dark:bg-amber-500/10">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              ...
              <button
                onClick={() =>
                  navigate("/packages?status=POR%20CONFIRMAR")
                }
            ...
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-white/50">
                  Últimos movimientos de paquetes
                </p>
              </div>
